// Configuration
let webhookUrl = localStorage.getItem('n8nWebhookUrl') || 'http://localhost:8080/webhook-proxy';
...
async function sendToN8N(message) {
    if (!webhookUrl) {
        addMessage("⚠ Please configure your n8n webhook URL in settings", false);
        return;
    }
    ...
    try {
        ...
        // Handle the response from the proxy/n8n
        ...
        setTimeout(() => {
            addMessage(`⚠ Connection error: ${error.message}. Please check your webhook configuration.`, false);
...
window.addEventListener('load', () => {
    ...
    // Check if webhook is configured and show appropriate message
    if (webhookUrl === 'http://localhost:8080/webhook-proxy') {
        setTimeout(() => {
            addMessage("👋 Welcome! The system is configured to use the local webhook proxy. Make sure to start the Python server with: python server.py", false);
        }, 1500);
    } else if (!webhookUrl) {
        setTimeout(() => {
            addMessage("👋 Welcome! Please configure your n8n webhook URL by clicking the settings button (⚙) in the top right corner.", false);
        }, 1500);
    }
});
